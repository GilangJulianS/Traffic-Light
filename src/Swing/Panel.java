...
public class Panel extends JPanel implements Runnable, MouseListener, MouseMotionListener, KeyListener{
	...
	private AffineTransform left, down, right, top, single, transButton;
	...
	public void init(){
		...
		transButton = new AffineTransform();
		...
		buttonArea = new Rectangle(130, 450, 100, 100);
		random = new Random();
		unset = true;
		hoverButton = false;
		buttonPushed = false;
		...
		transButton.translate(130, 450);
		transButton.scale(0.3, 0.3);
		//switchFullScreen();
	...
	public void update(Graphics g){
		...
		if(unset && merah.getWidth(null)!=-1){
			...
			left.translate(235, 233);
			left.rotate(Math.toRadians(90), centerx, centery);
			left.scale(0.7, 0.7);
			right.translate(439, 413);
			right.rotate(Math.toRadians(270), centerx, centery);
			right.scale(0.7, 0.7);
			down.translate(260, 411);
			down.scale(0.7, 0.7);
			top.translate(410, 207);
			top.scale(0.7, 0.7);
			single.translate(412, 413);
	...
	public void paintComponent(Graphics g){
		...
		if(hoverButton)
			g2d.drawImage(pushButton, transButton, this);
		else
			g2d.drawImage(button, transButton, this);
